...
void test_eight_bit_palette()
{
    test::section("eight-bit CPC palette");

    constexpr int width = 17;
    constexpr int height = 3;
    std::vector<png::colour> palette;
    for (int i = 0; i < 32; ++i) {
        palette.push_back({static_cast<std::uint8_t>(i * 7),
                           static_cast<std::uint8_t>(255 - i * 7),
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x)
            pixels.push_back(static_cast<std::uint8_t>((x + y * 11) & 31));
    }

    const std::vector<std::uint8_t> file =
        png::encode_indexed(pixels, width, height, palette, 1);
    const parsed_png parsed = parse_png(file);
    test::check(parsed.ok, "a 32-colour image parses");
    test::check_eq(parsed.bit_depth, 8,
                   "more than sixteen colours select eight-bit indices");
    test::check_eq(static_cast<long long>(parsed.palette_entries), 32,
                   "all CPC hardware colours are retained");

    const std::size_t expected_size =
        static_cast<std::size_t>((width + 1) * height);
    std::vector<std::uint8_t> inflated(expected_size + 32);
    mz_ulong inflated_size = static_cast<mz_ulong>(inflated.size());
    const int status = mz_uncompress(
        inflated.data(), &inflated_size, parsed.image_data.data(),
        static_cast<mz_ulong>(parsed.image_data.size()));
    test::check_eq(status, MZ_OK, "eight-bit image data inflates");
    test::check_eq(static_cast<long long>(inflated_size),
                   static_cast<long long>(expected_size),
                   "eight-bit scan lines have one byte per pixel");

    bool pixels_match = true;
    for (int y = 0; y < height && pixels_match; ++y) {
        const std::size_t row = static_cast<std::size_t>(y) * (width + 1);
        if (inflated[row] != 0) {
            pixels_match = false;
            break;
        }
        for (int x = 0; x < width; ++x) {
            if (inflated[row + 1 + static_cast<std::size_t>(x)] !=
    ...
    test::check(pixels_match,
                "every eight-bit palette index survives the round trip");
}
...
void test_compression_is_worthwhile()
{
    ...
    // a CPC screen is mostly flat colour, which is exactly what
