...
//
std::string describe_flags(cpc::u8 flags)
{
...
public:
    ...
    {
        cpc::cpu_registers regs = machine().registers();
        bool modified = false;

        const auto apply16 = [&](const char *field, cpc::u16 &slot) {
            const auto value = read_int_in(arguments[field], 0, 0xffff);
            if (value && !arguments[field].is_null()) {
                slot = static_cast<cpc::u16>(*value);
                modified = true;
            }
        };

        const auto apply8 = [&](const char *field, cpc::u8 &slot,
                                std::int64_t high) {
            const auto value = read_int_in(arguments[field], 0, high);
            if (value && !arguments[field].is_null()) {
                slot = static_cast<cpc::u8>(*value);
                modified = true;
            }
        };
        ...
        const cpc::cpu_registers now = machine().registers();
        ...
        // one a caller means.
        const cpc::u16 reported_pc =
            modified ? now.pc : machine().instruction_address();

        const auto flags = static_cast<cpc::u8>(now.af & 0xff);
        const auto accumulator =
            static_cast<cpc::u8>((now.af >> 8) & 0xff);
...
public:
    ...
    mcp::tool_result invoke(const json::value &arguments) override
    {
        ...
        cpc::breakpoint_set &set = machine().breakpoints();
...
private:
    mcp::tool_result add(const json::value &arguments,
                         cpc::breakpoint_set &set)
    {
        const std::string kind_name =
            arguments["kind"].as_string("execute");
        const auto kind =
            cpc::breakpoint_kind_from_name(kind_name);
        if (!kind)
        ...
        std::optional<cpc::u8> value;
        if (!arguments["value"].is_null()) {
            const auto raw = read_int_in(arguments["value"], 0, 0xff);
            if (!raw)
                return mcp::tool_result::failure(
                    "'value' must be between 0 and 255");
            value = static_cast<cpc::u8>(*raw);
        }

        const int id = set.add(*kind, static_cast<cpc::u16>(*address),
                               value);
    ...
    mcp::tool_result list(const cpc::breakpoint_set &set)
    {
        const std::vector<cpc::breakpoint> &items = set.all();
        ...
        for (const cpc::breakpoint &item : items) {
            json::value entry = json::value::make_object();
            entry.set("id", json::value(item.id));
            entry.set("kind", json::value(cpc::breakpoint_kind_name(
                                  item.kind)));
            ...
            text += std::to_string(item.id) + ": " +
                    cpc::breakpoint_kind_name(item.kind) + " at " +
                    hex16(item.address) +
