...
json::value structured_status(const cpc::tape_status &tape)
{
...
std::string summary(const cpc::tape_status &tape)
{
    if (!tape.loaded)
    ...
    if (tape.stopped_by_command)
        state = "stopped by a CDT command";
...
public:
    ...
    {
        return "Control the TAP or CDT cassette transport. Use status, "
