...
std::optional<std::vector<cpc::u8>> read_bytes(
    const json::value &value)
{
    ...
    if (value.is_array()) {
        for (const json::value &item : value.elements()) {
            ...
            if (!byte || *byte < 0 || *byte > 255)
                return std::nullopt;
            bytes.push_back(static_cast<cpc::u8>(*byte));
        }
        return bytes;
    }
    ...
    if (value.is_string()) {
        ...
        for (std::size_t i = 0; i < digits.size(); i += 2) {
            ...
            bytes.push_back(static_cast<cpc::u8>(
...
std::string hex_dump(cpc::u16 address,
                     const std::vector<cpc::u8> &data)
{
    ...
    for (std::size_t offset = 0; offset < data.size();
        ...
        for (std::size_t i = 0; i < count; ++i) {
            const cpc::u8 byte = data[offset + i];
...
machine_tool::machine_tool(cpc::machine &target) : machine_(target)
{
}

cpc::machine &machine_tool::machine() const { return machine_; }
...
json::value machine_tool::machine_state() const
{
    ...
    state.set("halted", json::value(machine_.halted()));
    return state;
}
