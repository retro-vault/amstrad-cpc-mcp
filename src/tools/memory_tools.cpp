...
public:
    ...
    {
        return "Read CPU-visible bytes from the CPC address space and "
               "return them as a "
               "hex dump plus a byte array. Reading has no side "
               "effects and consumes no emulated time. Addresses wrap "
               "at 65535. ROM paging and 6128 RAM banking are honoured.";
    }
    ...
    mcp::tool_result invoke(const json::value &arguments) override
    {
        ...
        const auto start = static_cast<cpc::u16>(*address);
        const std::vector<cpc::u8> data = machine().read_memory(
            start, static_cast<std::size_t>(*length));

        json::value bytes = json::value::make_array();
        for (const cpc::u8 byte : data)
...
public:
    ...
    std::string description() const override
    {
        return "Write bytes through the CPC's current memory map. Writes "
               "under a visible ROM reach the underlying RAM, exactly as "
               "Z80 writes do. Consumes no emulated time.";
    }
    ...
    mcp::tool_result invoke(const json::value &arguments) override
    {
        ...
        const std::size_t written = data->size();
