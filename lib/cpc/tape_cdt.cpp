//
// CPC Digital Tape container parsing, control flow and pulse decoding.
//
...
std::string block_error(const cdt_block &block, const std::string &message)
{
    char id[8];
    std::snprintf(id, sizeof id, "%02X", block.id);
    return "CDT block 0x" + std::string(id) + " at byte " +
           std::to_string(block.file_offset) + ": " + message;
}

bool take_block(std::span<const u8> data, std::size_t &offset,
                cdt_block &block, std::string &error)
{
    ...
    const auto sized16 = [&](std::size_t prefix, std::size_t at,
        ...
        if (!need(prefix))
            return false;
        const std::size_t count = cdt_u16(data.subspan(offset), at);
        length = prefix + count * scale;
        return true;
    };
    const auto sized24 = [&](std::size_t prefix, std::size_t at) {
        if (!need(prefix))
            return false;
        length = prefix + cdt_u24(data.subspan(offset), at);
        return true;
    };
    const auto sized32 = [&](std::size_t prefix, std::size_t at) {
        ...
        length = prefix + static_cast<std::size_t>(
                              cdt_u32(data.subspan(offset), at));
        return true;
    };
...
bool parse_blocks(std::span<const u8> data, std::vector<cdt_block> &blocks,
                  std::string &error)
{
    if (data.size() < 10 || !std::equal(signature.begin(), signature.end(),
                                         data.begin())) {
        error = "not a CDT image (missing ZXTape signature)";
        return false;
    }
    if (data[8] > 1) {
        error = "unsupported CDT major version " + std::to_string(data[8]);
        return false;
    }
...
{
    const auto bytes = body.subspan(4, cdt_u16(body, 2));
    ...
    return out.append_pulses(2168, pilot_count) &&
           ...
           out.append_pause(cdt_u16(body, 0));
}

bool append_turbo(builder &out, std::span<const u8> body)
{
    return out.append_pulses(cdt_u16(body, 0), cdt_u16(body, 10)) &&
           out.append_pulse(cdt_u16(body, 2)) &&
           out.append_pulse(cdt_u16(body, 4)) &&
           out.append_data(body.subspan(18, cdt_u24(body, 15)),
                           cdt_u16(body, 6), cdt_u16(body, 8), body[12]) &&
           out.append_pause(cdt_u16(body, 13));
}

bool append_pure_data(builder &out, std::span<const u8> body)
{
    return out.append_data(body.subspan(10, cdt_u24(body, 7)),
                           cdt_u16(body, 0), cdt_u16(body, 2), body[4]) &&
           out.append_pause(cdt_u16(body, 5));
}

bool append_direct(builder &out, std::span<const u8> body)
{
    const u16 sample_length = cdt_u16(body, 0);
    const u8 last_bits = body[4];
    const auto samples = body.subspan(8, cdt_u24(body, 5));
    if (sample_length == 0)
        return false;
    ...
    return out.append_pause(cdt_u16(body, 2));
}
...
void extract_archive_title(const cdt_block &block, std::string &title)
{
...
bool first_selection(const cdt_block &block, i16 &choice,
                     std::string &error)
{
    ...
    for (u8 i = 0; i < body[2]; ++i) {
        ...
        const i16 target = static_cast<i16>(cdt_u16(body, offset));
...
bool decode_cdt(std::span<const u8> data, image &result,
                std::string &error)
{
    std::vector<cdt_block> blocks;
    if (!parse_blocks(data, blocks, error))
        return false;

    std::size_t data_blocks = 0;
    std::string title;
    for (const cdt_block &block : blocks) {
    ...
    while (pc < blocks.size()) {
        if (++executions > max_block_executions) {
            error = "CDT control flow exceeds the 1000000 block limit";
            return false;
        }
        const cdt_block &block = blocks[pc];
        const auto body = std::span<const u8>(block.body);
        ...
        case 0x12:
            ok = out.append_pulses(cdt_u16(body, 0), cdt_u16(body, 2));
            break;
        case 0x13:
            for (std::size_t i = 0; ok && i < body[0]; ++i)
                ok = out.append_pulse(cdt_u16(body, 1 + i * 2));
            break;
        ...
        case 0x17:
            ...
            return false;
        case 0x18: ok = append_cdt_csw(out, block, error); break;
        case 0x19: ok = append_cdt_generalized(out, block, error); break;
        case 0x20:
            ok = cdt_u16(body, 0) == 0
                     ? out.append_stop()
                     : out.append_pause(cdt_u16(body, 0));
        ...
        case 0x23: {
            const auto target = relative_target(
                pc, static_cast<i16>(cdt_u16(body, 0)), blocks.size());
            if (!target) {
        ...
        case 0x24:
            ...
            loops.push_back({pc + 1, cdt_u16(body, 0)});
            break;
        case 0x25:
            if (loops.empty()) {
        ...
        case 0x26: {
            call_state call;
            call.origin = pc;
            const std::size_t count = cdt_u16(body, 0);
            for (std::size_t i = 0; i < count; ++i)
                call.targets.push_back(
                    static_cast<i16>(cdt_u16(body, 2 + i * 2)));
    ...
    if (!loops.empty()) {
        error = "CDT loop start has no loop end";
        return false;
    }
    if (!calls.empty()) {
        error = "CDT call sequence has no return";
        return false;
    }
