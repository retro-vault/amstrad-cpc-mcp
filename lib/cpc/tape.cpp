...
// CDT pulse counts are defined against the format's 3.5 MHz reference clock.
// The CPC cassette input is sampled by the 4 MHz CPU clock, so converting
// every span by 8/7 preserves its wall-clock duration. Rounding each span
// independently also keeps every edge within half a CPC T-state.
bool convert_to_cpc_clock(image &decoded, std::string &error)
{
    u64 total = 0;
    for (event &item : decoded.events) {
        if (item.kind != event_kind::span)
            continue;
        if (item.duration >
            (std::numeric_limits<u64>::max() - 3) / 8) {
            error = "tape duration overflows while converting to 4 MHz";
            return false;
        }
        item.duration = (item.duration * 8 + 3) / 7;
        if (total > std::numeric_limits<u64>::max() - item.duration) {
            error = "tape duration overflows while converting to 4 MHz";
            return false;
        }
        total += item.duration;
    }
    decoded.duration_tstates = total;
    return true;
}
...
public:
                    ...
                    std::string &error)
    {
        ...
        if (!tape_internal::decode_tap(data, decoded, error))
            return false;
        if (!tape_internal::convert_to_cpc_clock(decoded, error))
            return false;
    ...
    {
        tape_internal::image decoded;
        if (!tape_internal::decode_cdt(data, decoded, error))
            return false;
        if (!tape_internal::convert_to_cpc_clock(decoded, error))
            return false;
        install(std::move(decoded), autoplay);
...
bool tape_deck::insert_cdt(std::span<const u8> data, bool autoplay,
                           std::string &error)
{
    return impl_->insert_cdt(data, autoplay, error);
}
