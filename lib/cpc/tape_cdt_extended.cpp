//
// CDT sampled-waveform and generalized-data block decoding.
//
...
bool extended_error(const cdt_block &block, const std::string &message,
                    std::string &error)
{
    error = "CDT block at byte " + std::to_string(block.file_offset) +
            ": " + message;
    return false;
}
...
bool append_csw_rle(builder &out, std::span<const u8> encoded,
                    ...
                    const cdt_block &block, std::string &error)
{
    std::size_t offset = 0;
        ...
        if (samples == 0) {
            if (encoded.size() - offset < 4)
                return extended_error(block,
                                      ...
                                      error);
            samples = cdt_u32(encoded, offset);
            offset += 4;
        }
        if (samples == 0)
                  ...
                  const cdt_block &block, std::string &error)
{
    ...
    for (std::size_t i = 0; i < count; ++i) {
        ...
        for (std::size_t pulse = 0; pulse < max_pulses; ++pulse) {
            symbol.pulses.push_back(cdt_u16(body, offset));
            offset += 2;
        }
                    ...
                    std::string &error)
{
    ...
    const u32 sample_rate = cdt_u24(body, 6);
    const u8 compression = body[9];
    const u32 pulse_count = cdt_u32(body, 10);
    if (sample_rate == 0)
    ...
    if (!append_csw_rle(out, rle, pulse_count, sample_rate, block, error))
        return false;
    if (!out.append_pause(cdt_u16(body, 4))) {
        error = out.error();
        return false;
    }
    return true;
                            ...
                            std::string &error)
{
    ...
    if (pilot_total != 0) {
        ...
        for (u32 record = 0; record < pilot_total; ++record) {
            ...
            const u16 repeats = cdt_u16(body, offset);
            offset += 2;
