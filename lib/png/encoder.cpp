...
constexpr std::uint8_t four_bit_depth = 4;
constexpr std::uint8_t eight_bit_depth = 8;
...
//
// Build raw scan lines: a filter byte followed by packed palette indices.
//
std::vector<std::uint8_t> build_raw_lines(
    std::span<const std::uint8_t> pixels, int width, int height,
    std::size_t palette_size, int scale, std::uint8_t depth)
{
    const int scaled_width = width * scale;
    const int scaled_height = height * scale;
    const std::size_t row_bytes = depth == four_bit_depth
    ...
    for (int y = 0; y < scaled_height; ++y) {
        ...
        if (depth == eight_bit_depth) {
            for (int x = 0; x < scaled_width; ++x)
                raw.push_back(clamp_index(line[x / scale]));
            continue;
        }

        // Two four-bit pixels are packed per byte, high nibble first. An
    ...
    std::span<const colour> palette, int scale)
{
    ...
    const std::uint8_t depth = palette.size() <= 16
        ? four_bit_depth : eight_bit_depth;

    const std::vector<std::uint8_t> raw =
        build_raw_lines(pixels, width, height, palette.size(), scale, depth);
