...
/*#
    ...
    Locally altered for amstrad-cpc-mcp: CPC664 selection, selectable CRTC,
    cassette/printer wiring, model-correct floppy decode, C++ portability and
    snapshot bounds corrections. See lib/chips/UPSTREAM.md.
...
typedef struct {
    ...
    mc6845_type_t crtc_type;         // fitted CRTC type (0, 1 or 2)
...
void cpc_init(cpc_t* sys, const cpc_desc_t* desc) {
    ...
    } else if (CPC_TYPE_664 == desc->type) {
        CHIPS_ASSERT(desc->roms.cpc664.os.ptr && (desc->roms.cpc664.os.size == 0x4000));
        CHIPS_ASSERT(desc->roms.cpc664.basic.ptr && (desc->roms.cpc664.basic.size == 0x4000));
        CHIPS_ASSERT(desc->roms.cpc664.amsdos.ptr && (desc->roms.cpc664.amsdos.size == 0x4000));
        memcpy(sys->rom_os, desc->roms.cpc664.os.ptr, 0x4000);
        memcpy(sys->rom_basic, desc->roms.cpc664.basic.ptr, 0x4000);
        memcpy(sys->rom_amsdos, desc->roms.cpc664.amsdos.ptr, 0x4000);
    ...
    memset(&ay_desc, 0, sizeof(ay_desc));
    ay_desc.type = AY38910_TYPE_8912;
    ay_desc.tick_hz = _CPC_FREQUENCY / 4;
    ay_desc.sound_hz = _CPC_DEFAULT(desc->audio.sample_rate, 44100);
    ay_desc.magnitude = _CPC_DEFAULT(desc->audio.volume, 0.5f);
    ay_desc.in_cb = _cpc_psg_in;
    ay_desc.out_cb = _cpc_psg_out;
    ay_desc.user_data = sys;
    ay38910_init(&sys->psg, &ay_desc);
    mc6845_init(&sys->crtc, desc->crtc_type);
    mem_init(&sys->mem);
    am40010_desc_t ga_desc;
    memset(&ga_desc, 0, sizeof(ga_desc));
    ga_desc.cpc_type = (CPC_TYPE_6128 == sys->type)
        ? AM40010_CPC_TYPE_6128 : AM40010_CPC_TYPE_464;
    ga_desc.bankswitch_cb = _cpc_bankswitch;
    ga_desc.cclk_cb = _cpc_cclk;
    ga_desc.ram.ptr = &sys->ram[0][0];
    ga_desc.ram.size = sizeof(sys->ram);
    ga_desc.framebuffer.ptr = &sys->fb[0];
    ga_desc.framebuffer.size = sizeof(sys->fb);
    ga_desc.user_data = sys;
    am40010_init(&sys->ga, &ga_desc);

    upd765_desc_t fdc_desc;
    memset(&fdc_desc, 0, sizeof(fdc_desc));
    ...
    if (cpu_pins & Z80_MREQ) {
        ...
        */
        if ((cpu_pins & Z80_A11) == 0) {
            ...
            sys->tape_motor_on = 0 != (I8255_GET_PC(ppi_pins) & (1<<4));
            sys->tape_output = 0 != (I8255_GET_PC(ppi_pins) & (1<<5));
        ...
        */
        ...
        /*
            Centronics printer data latch. The internal decoder watches A12
            only, so writes can overlap other partially decoded devices.
        */
        if (((cpu_pins & Z80_A12) == 0) && (cpu_pins & Z80_WR)) {
            sys->printer_data = Z80_GET_DATA(cpu_pins);
        }
        /*
        ...
        */
        ...
        if ((CPC_TYPE_464 != sys->type) &&
            ((cpu_pins & (Z80_A10|Z80_A8|Z80_A7)) == 0)) {
            if (cpu_pins & Z80_WR) {
                fdd_motor(&sys->fdd, 0 != (Z80_GET_DATA(cpu_pins) & 1));
            }
        } else if ((CPC_TYPE_464 != sys->type) &&
                   ((cpu_pins & (Z80_A10|Z80_A8|Z80_A7)) == Z80_A8)) {
...
chips_display_info_t cpc_display_info(cpc_t* sys) {
    chips_display_info_t res = {0};
    res.frame.dim.width = AM40010_FRAMEBUFFER_WIDTH;
    res.frame.dim.height = AM40010_FRAMEBUFFER_HEIGHT;
    res.frame.buffer.ptr = sys ? sys->fb : 0;
    res.frame.buffer.size = AM40010_FRAMEBUFFER_SIZE_BYTES;
    res.frame.bytes_per_pixel = 1;
    res.screen.x = 0;
    res.screen.y = 0;
    res.screen.width = (sys && sys->ga.dbg_vis)
        ? AM40010_FRAMEBUFFER_WIDTH : AM40010_DISPLAY_WIDTH;
    res.screen.height = (sys && sys->ga.dbg_vis)
        ? AM40010_FRAMEBUFFER_HEIGHT : AM40010_DISPLAY_HEIGHT;
    res.palette.ptr = sys ? sys->ga.hw_colors : 0;
    res.palette.size = AM40010_NUM_HWCOLORS * sizeof(uint32_t);
