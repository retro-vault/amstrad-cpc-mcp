...
// one call return a mix of text, images and audio. The screen tool returns
// a PNG beside its summary, and audio_stop can do the same with a WAV.
//
...
public:
    ...
    //
    // Parameters:
    //      base64      - the audio bytes, base64 encoded.
    //      mime_type   - for instance "audio/wav".
    //
    static content_block audio(std::string base64, std::string mime_type);
...
//
// Encode bytes as base64, for binary content blocks.
//
// Parameters:
//      data        - the raw bytes.
//
